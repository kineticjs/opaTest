...
(function (global$1) {
  ...
  // adds to the beginning if the queue
  ...
  Test.prototype = {
  	...
  	queue: function queue() {
		  ...
		  // defines a subQueue,
		  // composed of the test itself plus the callbacks for 'before' and 'after' the test execution

		  // adds the subQueue to the FRONT of the main queue
		  // (to be executed before the remaining tests)
  		function runTest() {
  			...
  			ProcessingQueue.addImmediate([function () {
  				...
  				test.run(); // runs the actual test from the app
  ...
  function test(testName, callback) {
  	...
  	var newTest = new Test({
  		testName: testName,
  		callback: callback // the test content defined by the app
  	});
  ...
  extend(QUnit, {
  	...
  	load: function load() {
  		...
  		if (!runStarted) {
  			...
  			if (config.autostart) {
				  // proceed to process the queue of tests
  				scheduleBegin();
  			}
  		}
  	},
  ...
  function begin() {
  	...
  	config.blocking = false;
	// begins to process the tests in the queue
  	ProcessingQueue.advance();
  }
